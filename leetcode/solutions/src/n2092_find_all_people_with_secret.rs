...
impl Solution {
	pub fn find_all_people(n: i32, mut meetings: Vec<Vec<i32>>, first_person: i32) -> Vec<i32> {
		...
		meetings.sort_unstable_by_key(|v| v[2]);
		let mut set: Vec<_> = (0..n).collect();
		union(&mut set, 0, first_person);
		let mut prv = -1;
		let mut que = vec![];
		for v in meetings {
			let (a, b, t) = (v[0], v[1], v[2]);
			if t != prv {
				for i in que.drain(0..) {
					if find(&mut set, i) != find(&mut set, 0) {
						set[i as usize] = i;
					}
				}
				prv = t;
		...
		(0..n)
			.filter(|i| find(&mut set, *i) == find(&mut set, 0))
			.collect()
	}
}
