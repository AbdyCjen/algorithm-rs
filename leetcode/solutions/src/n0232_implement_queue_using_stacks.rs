...
impl MyQueue {
	...
	fn push(&mut self, x: i32) { self.in_st.push(x); }

	fn pop(&mut self) -> i32 {
		match self.out_st.pop() {
			Some(i) => i,
			_ => {
				while let Some(i) = self.in_st.pop() {
					self.out_st.push(i);
				}
				self.out_st.pop().unwrap()
			}
		}
	}

	fn peek(&mut self) -> i32 {
		match self.out_st.last() {
			Some(i) => *i,
			_ => {
				while let Some(i) = self.in_st.pop() {
					self.out_st.push(i);
				}
				*self.out_st.last().unwrap()
			}
		}
	}
