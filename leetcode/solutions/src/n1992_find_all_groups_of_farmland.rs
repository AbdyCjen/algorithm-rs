		...
		for i in 0..m {
			for j in 0..n {
				if land[i][j] == 1 {
					...
					ans.push(vec![i as i32, j as i32, (x - 1) as i32, (y - 1) as i32]);
				}
			}
		}
		ans
...
mod tests {
	...
	fn test_1992() {
		assert_eq!(
			Solution::find_farmland(matrix![[1, 1], [1, 1]]),
			matrix![[0, 0, 1, 1]]
		);
		assert_eq!(
			Solution::find_farmland(matrix![[1, 0, 0], [0, 1, 1], [0, 1, 1]]),
			matrix![[0, 0, 0, 0], [1, 1, 2, 2]]
		);
	}
}
