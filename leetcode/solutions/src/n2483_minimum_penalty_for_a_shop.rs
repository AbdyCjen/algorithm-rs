...
impl Solution {
	pub fn best_closing_time(s: String) -> i32 {
		let min_pen = s.bytes().filter(|i| *i == b'Y').count() as i32;
		(1..)
			.zip(s.bytes())
			.fold(
				(0, min_pen, min_pen),
				|(ans, cur, min_pen), (i, c)| match c {
					b'Y' if cur <= min_pen => (i, cur - 1, cur - 1),
					b'Y' => (ans, cur - 1, min_pen),
					_ => (ans, cur + 1, min_pen),
				},
			)
			.0
