...
impl Solution {
	pub fn can_eat(candies: Vec<i32>, queries: Vec<Vec<i32>>) -> Vec<bool> {
		let mut candies: Vec<_> = candies
			.iter()
			.scan(0_i64, |f, &c| {
				*f += c as i64;
				Some(*f)
		...
		queries
			.iter()
			.map(|q| {
				let (i, day, cap) = (q[0] as usize, q[1] as i64, q[2] as i64);
				candies[i + 1] > day && candies[i] < cap * (day + 1)
			})
			.collect()
	}
}
...
mod tests {
	...
	fn test_1744() {
		assert_eq!(
			Solution::can_eat(vec![7, 11, 5, 3, 8], matrix![[2, 2, 6]],),
			[false],
		);
	}
}
