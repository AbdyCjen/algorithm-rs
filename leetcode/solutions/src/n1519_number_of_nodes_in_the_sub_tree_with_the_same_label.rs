...
impl Solution {
	pub fn count_sub_trees(n: i32, edges: Vec<Vec<i32>>, labels: String) -> Vec<i32> {
		let mut tree = labels
			...
			.collect::<Vec<_>>();
		for e in edges {
			tree[e[0] as usize].1.push(e[1]);
			tree[e[1] as usize].1.push(e[0]);
		}
		let mut ans = vec![0; n as usize];
		Self::dfs(0, 0, &tree, &mut ans);
		ans
	}

	fn dfs(n: i32, p: i32, tree: &[(u8, Vec<i32>)], ans: &mut [i32]) -> [i32; 26] {
		let (c, neis) = &tree[n as usize];
		let mut cnt = [0; 26];
		for &nei in neis {
			if nei != p {
				for (c1, c2) in cnt.iter_mut().zip(Self::dfs(nei, n, tree, ans)) {
					*c1 += c2;
				}
			}
		}
		cnt[(c - b'a') as usize] += 1;
		ans[n as usize] = cnt[(c - b'a') as usize];
		cnt
	}
}
