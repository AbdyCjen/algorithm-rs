...
impl Solution {
	pub fn min_wasted_space(mut packages: Vec<i32>, boxes: Vec<Vec<i32>>) -> i32 {
		...
		for mut bs in boxes {
			...
			for b in bs {
				let pack = &packages[start_from..];
				//after rust 1.52
				//let cur = pack.partition_point(|&j| j <= b);
				let cur = pack
					.binary_search(&b)
					.map(|mut i| {
						while pack.get(i) == Some(&b) {
							i += 1;
						}
						i
					})
				...
				total_box += (cur * b as usize) as i64;
				start_from += cur;
			}

			if start_from == packages.len() {
				ans = ans.min(total_box - total_space);
			}
		}
