...
impl Solution {
	pub fn number_of_beams(bank: Vec<String>) -> i32 {
		...
		let mut prv = 0;
		for s in bank {
			let n = s.bytes().filter(|c| *c == b'1').count() as i32;
			if n > 0 {
				ans += prv * n;
				prv = n;
			}
...
mod tests {
	...
	fn test_2125() {
		assert_eq!(
			Solution::number_of_beams(vec_string!["000", "111", "000"]),
			0
		);
		assert_eq!(
			Solution::number_of_beams(vec_string!["011001", "000000", "010100", "001000"]),
			8
		);
	}
}
