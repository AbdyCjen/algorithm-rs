...
impl Solution {
	pub fn time_required_to_buy(tiks: Vec<i32>, k: i32) -> i32 {
		let mut ans = tiks[k as usize];
		for i in 0..k as usize {
			ans += tiks[i].min(tiks[k as usize]);
		}
		for i in k as usize + 1..tiks.len() {
			ans += tiks[k as usize].min(tiks[i] - 1);
		}
		ans
	}
}
