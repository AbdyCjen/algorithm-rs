...
impl Solution {
	pub fn most_profitable_path(edges: Vec<Vec<i32>>, mut bob: i32, mut amount: Vec<i32>) -> i32 {
		let n = amount.len();
		let mut neighs = vec![vec![]; n];
		for e in edges {
			neighs[e[0] as usize].push(e[1]);
			neighs[e[1] as usize].push(e[0]);
		}
		let mut parent = vec![-1; n];
		let mut dq = vec![0];
		parent[0] = 0;
		while !dq.is_empty() {
			for n in dq.split_off(0) {
				for &j in &neighs[n as usize] {
		...
		let mut ans = i32::MIN;
		while !dq.is_empty() {
			for (n, amt) in dq.split_off(0) {
				let neig = &neighs[n as usize];
				if neig.len() == 1 && n != 0 {
					ans = ans.max(amt);
					continue;
				}
				for &ne in neig {
					if ne != parent[n as usize] {
						if ne == bob {
							dq.push((ne, amt + amount[ne as usize] / 2));
							amount[ne as usize] = 0;
						} else {
							dq.push((ne, amt + amount[ne as usize]));
							amount[ne as usize] = 0;
						}
					}
				}
...
mod tests {
	...
	fn test_2467() {
		assert_eq!(
			Solution::most_profitable_path(matrix![[0, 1]], 1, vec![-7280, 2350]),
			-7280
		);
		assert_eq!(
			Solution::most_profitable_path(
				matrix![[0, 1], [1, 2], [1, 3], [3, 4]],
				3,
				vec![-2, 4, 2, -4, 6]
			),
			6
